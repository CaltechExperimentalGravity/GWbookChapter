...
output (or are expected to in future detectors).
\begin{figure}[t]
\centering
...
\toprule
{} &                     Risk Description & System &  Prob. (\%) &  Cost (M\$) &  Time (m) &  BNS (\% Range) &  BBH (\% Range) &  Total Severity \\
%\midrule
...
extra diagnostics would make diagnoses of rare problems easier to make:

\begin{itemize}
  \item Since the GW detection band overlaps mainly with the audio band, the
  ...
  \item Chapter\,\ref{COP} describes the higher order spatial modes (HOM) of
