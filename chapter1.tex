...
\begin{verbatim}
\begin{figure}
\centerline{\includegraphics[width=3.6cm]{bk-fig1}}
...
\verb|\documentclass[onethmnum]{ws-book975x65}|.
...
The WSPC prefers ``Book Title'' as even page header and ``Chapter
Title'' as odd page header. However, authors can pick their required
style by switching the class options while loading the
\verb|ws-book975x65| document class in the preamble.

\begin{table}[ht]
\centerline
{\tablefont\begin{tabular}{@{}lll@{}}\toprule
Class options&Even page& Odd page\\\colrule
{\tt$\backslash$documentclass[acrhead]}\{{\tt{ws-book975x65}}\}$^{\textrm a}$ & Author Name(s) & Chapter Title\\
{\tt$\backslash$documentclass[csrhead]}\{{\tt{ws-book975x65}}\} & Chapter Title & Section Title\\
{\tt$\backslash$documentclass}\{{\tt{ws-book975x65}}\} & Book Title
...
\verb|ws-book-van.bst| for (numbered) references. \Cref{ch2} of this document has more on \btex.
